...
class Graph:
    ...
    def create_trajectory(self):
        ...
        #     matrix_list[ -coord[1]-1 ][ coord[0] ] = PROJECTILE

        matrix = ["".join(line) for line in matrix_list]
        # matrix_list = [''.join(this_list) for this_list in matrix_list]

        # matrix = [y_axis_tick+ ''.join(line) for line in matrix_list]
        # matrix.append(f' {"":{x_axis_tick}>{len(matrix[0])-1}}')
